...
// Implementación de la comprobación de la consulta 1 (alumnos de una asignatura de una titulación)
...
public class Query1 {
	...
	static final String PSUBJECT = "psubject";


	// COMPROBACIÓN DE LAS LLAMADAS A SINTPROF
	// es sólo para el profesor, no es necesario traducir

	public static void doGetC1CheckSintprofCalls(HttpServletRequest request, HttpServletResponse response, String lang)
						throws IOException, ServletException
	{
		...
		try {
			...
			out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: error al preguntar por el estado de sintprof: <br>");
		...
		try {
			...
			cf = e.getCheckerFailure();
			out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: ExcepcionChecker pidiendo los errores: <br>");
			out.println(cf.toHTMLString());
			out.println("</h4>");
			CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
			return;
			...
			out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: Exception pidiendo los errores: "+ex.toString()+"</h4>");
			CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
			return;
    }
		...
		String qs = "?auto=true&"+CommonSINT.PFASE+"=11&p="+CommonSINT.PPWD;
		String call = CommonEAChecker.servicioProf+qs;

		ArrayList<String> pDegrees;
		try {
			pDegrees = Query1.requestDegrees(call, lang);
			out.println("<h4>Degrees OK: "+pDegrees.size()+"</h4>");
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: ExcepcionChecker pidiendo los degrees a sintprof: <br>");
			out.println(cf.toHTMLString());
			out.println("</h4>");
			CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
			return;
		...
		for (int x=0; x < pDegrees.size(); x++) {
			...
			// pedimos los subjects de ese degree de sintprof
			// los degrees pueden tener caracteres no ASCII, hay que aplicar un URLencode

			try {
			    qs = "?auto=true&"+CommonSINT.PFASE+"=12&"+PDEGREE+"="+URLEncoder.encode(degreeActual, "utf-8")+"&p="+CommonSINT.PPWD;
					call = CommonEAChecker.servicioProf+qs;
			}
		  catch (UnsupportedEncodingException ex) {
					out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: UnsupportedEncodingException pidiendo los subjects: UTF-8 no soportado</h4>");
					CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
					return;
		  }
			...
			try {
				pSubjects = requestSubjectsDegree(call, lang);
				out.println("<h4>"+indent+degreeActual+": "+pSubjects.size()+"  OK</h4>");
			}
			catch (ExcepcionChecker e) {
				cf = e.getCheckerFailure();
				out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: ExcepcionChecker pidiendo los subjects de "+degreeActual+" a sintporof: <br>");
				out.println(cf.toHTMLString());
				out.println("</h4>");
				CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
				return;
	        ...
	        // vamos con la tercera fase, los students de un subject en un degree
	        // el bucle Y recorre todos los subjects

	        Subject subjectActual;

	        for (int y=0; y < pSubjects.size(); y++) {

		        	indent = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";

		        	subjectActual = pSubjects.get(y);

		    		// pedimos los students de ese subject de ese degree de sintprof
					  // los degrees y los subjects pueden tener caracteres no ASCII, hay que aplicar un URLencode

						try {
		    		    qs = "?auto=true&"+CommonSINT.PFASE+"=13&"+PDEGREE+"="+URLEncoder.encode(degreeActual, "utf-8")+"&"+PSUBJECT+"="+URLEncoder.encode(subjectActual.getNombre(), "utf-8")+"&p="+CommonSINT.PPWD;
							  call = CommonEAChecker.servicioProf+qs;
						}
						catch (UnsupportedEncodingException ex) {
								out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: UnsupportedEncodingException pidiendo los students a sintprof: UTF-8 no soportado</h4>");
								CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
								return;
					  }
		    		...
		    		try {
		    			pStudents = requestStudentsSubjectDegree(call, lang);
		    			out.println("<h4>"+indent+degreeActual+"+"+subjectActual.getNombre()+": "+pStudents.size()+"  OK</h4>");
		    		}
		    		catch (ExcepcionChecker e) {
						cf = e.getCheckerFailure();
		    			out.println("<h4 style='color: red'>doGetC1CheckSintprofCalls: ExcepcionChecker pidiendo los students a sintprof: <br>");
						  out.println(cf.toHTMLString());
		    			out.println("</h4>");
		    			CommonSINT.printEndPageChecker(out,  "0", esProfesor, MsgsEAMLChecker.CREATED, lang);
		    			return;
	...
	public static void doGetC1CorrectOneForm (HttpServletRequest request, HttpServletResponse response, String lang, int esProfesor)
						throws IOException
	{
		PrintWriter out = response.getWriter();
		...
		out.println("<h2>"+Msgs.getMsg(Msgs.CC03,lang)+" 1</h2>");   // CC03=Consulta
	...
	public static void doGetC1CorrectOneReport(HttpServletRequest request, HttpServletResponse response, String lang, int esProfesor)
						throws IOException, ServletException
	{
		...
		out.println("<h2>"+Msgs.getMsg(Msgs.CC03,lang)+" 1</h2>");  // CC03=Consulta 1
		out.println("<h3>"+Msgs.getMsg(Msgs.CC04,lang)+"</h3>");  // CC04 = corrigiendo un servicio
		...
		if ((alumnoP == null) || (servicioAluP == null)) {
			...
			CommonSINT.printEndPageChecker(out,  "12", esProfesor, MsgsEAMLChecker.CREATED, lang);
			return;
		}
		...
		try {
			Query1.correctC1OneStudent(request, usuario, alumnoP, servicioAluP, passwdAlu, lang);
		}
		catch (ExcepcionChecker e) {
			CheckerFailure cf = e.getCheckerFailure();
    ...
    private static void correctC1OneStudent (HttpServletRequest request,String usuario, String aluNum, String servicioAluP, String passwdAlu, String lang)
    			throws ExcepcionChecker
	{
		...
		try {
			CommonEAChecker.doOneCheckUpFiles(aluNum, "1", lang);  // 1 es el número de la consulta
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
		...
		qs = "?auto=true&"+CommonSINT.PFASE+"=11&p="+passwdAlu;
		call = servicioAluP+qs;

		ArrayList<String> xDegrees;
		try {
			xDegrees = Query1.requestDegrees(call, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setCodigo("20_DIFS");
		...
		try {
		     Query1.comparaDegrees(usuario, pDegrees, xDegrees, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setUrl(call);
		...
		for (int x=0; x < pDegrees.size(); x++) {
		  ...
		  // los degrees pueden tener caracteres no ASCII, hay que aplicar un URLencode
			...
			try {
				...
				cf = e.getCheckerFailure();
				cf.setCodigo("20_DIFS");
				cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("21",lang)+"subjects - "+degreeActual+" (sintprof)"); // error solicitando lista de
				throw new ExcepcionChecker(cf);
			}


			// pedimos los subjects de ese degree del sintX
			// los degrees pueden tener caracteres no ASCII, hay que aplicar un URLencode

			try {
			   qs = "?auto=true&"+CommonSINT.PFASE+"=12&"+PDEGREE+"="+URLEncoder.encode(degreeActual, "utf-8")+"&p="+passwdAlu;
				 call = servicioAluP+qs;
			 }
			 catch (UnsupportedEncodingException ex) {
				 CommonEAChecker.logCall(call);
			...
			try {
				xSubjects = requestSubjectsDegree(call, lang);
			}
			catch (ExcepcionChecker e) {
				cf = e.getCheckerFailure();
				cf.setCodigo("20_DIFS");
				cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("21",lang)+"subjects - "+degreeActual+" ("+usuario+")");  // error solicitando lista de
				throw new ExcepcionChecker(cf);
			}
			...
			try {
				Query1.comparaSubjects(usuario, degreeActual, pSubjects, xSubjects, lang);
			}
			catch (ExcepcionChecker e) {
				cf = e.getCheckerFailure();
				cf.setUrl(call);
				cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("22",lang)+"subjects ("+degreeActual+")"); // diferencias en la lista de
				throw new ExcepcionChecker(cf);
			}



	        // las listas de subjects para este degree son iguales
		    // si este degree es el de la consulta directa final, elegimos un subject al azar para la consulta directa final


	        if (degreeActual.equals(dqDegree)) {
	            posrandom = SintRandom.getRandomNumber(0, pSubjects.size()-1);
	            Subject pSub = pSubjects.get(posrandom);
	          	dqSubject = pSub.getNombre();
	        }



	        // vamos con la tercera fase, los students de un subject
	        // el bucle Y recorre todos los subjects
	        ...
	        for (int y=0; y < pSubjects.size(); y++) {
	    		...
	    		try {
	    			...
	    			cf = e.getCheckerFailure();
					cf.setCodigo("20_DIFS");
					cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("21",lang)+"students (sintprof)");  // error solicitando lista de
					throw new ExcepcionChecker(cf);
	    		}


	    		// pedimos los students de ese subject de ese degree del sintX
					// los degrees y los subjects pueden tener caracteres no ASCII, hay que aplicar un URLencode

					try {
	    		   qs = "?auto=true&"+CommonSINT.PFASE+"=13&"+PDEGREE+"="+URLEncoder.encode(degreeActual, "utf-8")+"&"+PSUBJECT+"="+URLEncoder.encode(subjectActual.getNombre(), "utf-8")+"&p="+passwdAlu;
	    		   call = servicioAluP+qs;
					}
 				  catch (UnsupportedEncodingException ex) {
						CommonEAChecker.logCall(call);
	    		...
	    		try {
	    			xStudents = requestStudentsSubjectDegree(call, lang);
	    		}
	    		catch (ExcepcionChecker e) {
	    			cf = e.getCheckerFailure();
					cf.setCodigo("20_DIFS");
					cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("21",lang)+"students ("+usuario+")"); // error solicitando lista de
					throw new ExcepcionChecker(cf);
	    		}
	    		...
	    		try {
	    			Query1.comparaStudents(usuario, degreeActual, subjectActual.getNombre(), pStudents, xStudents, lang);
				}
				catch (ExcepcionChecker e) {
					cf = e.getCheckerFailure();
					cf.setUrl(call);
					cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("22",lang)+"students ("+degreeActual+", "+subjectActual.getNombre()+")"); // diferencias en la lista de
					throw new ExcepcionChecker(cf);
				}
		...
		try {
			Query1.checkDirectQueryC1(CommonEAChecker.servicioProf, usuario, servicioAluP, dqDegree, dqSubject, passwdAlu, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setCodigo("20_DIFS");
	...
	private static void checkDirectQueryC1(String servicioProf, String usuario, String servicioAluP, String degree, String subject, String passwdAlu, String lang)
		throws ExcepcionChecker
	{
		String currentMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
		CheckerFailure cf;
		ArrayList<Student> pStudents, xStudents;
		String qs="", call="";

		// primero comprobamos que responde con el error apropiado si falta algún parámetro

  		try {
  			CommonEAChecker.checkLackParam(servicioAluP, passwdAlu, "13", PDEGREE, degree, PSUBJECT, subject, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setCodigo("20_DIFS");
		...
		try {
		    qs = "?auto=true&"+CommonSINT.PFASE+"=13&"+PDEGREE+"="+URLEncoder.encode(degree, "utf-8")+"&"+PSUBJECT+"="+URLEncoder.encode(subject, "utf-8")+"&p="+CommonSINT.PPWD;
				call = CommonEAChecker.servicioProf+qs;
		}
		catch (UnsupportedEncodingException ex) {
			CommonEAChecker.logCall(call);
   		...
   		try {
   			pStudents = Query1.requestStudentsSubjectDegree(call, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setCodigo("20_DIFS");
		...
		try {
		    qs = "?auto=true&"+CommonSINT.PFASE+"=13&"+PDEGREE+"="+URLEncoder.encode(degree, "utf-8")+"&"+PSUBJECT+"="+URLEncoder.encode(subject, "utf-8")+"&p="+passwdAlu;
				call = servicioAluP+qs;
		}
		catch (UnsupportedEncodingException ex) {
			CommonEAChecker.logCall(call);
  		...
  		try {
  			xStudents = Query1.requestStudentsSubjectDegree(call, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setCodigo("20_DIFS");
		...
		// comparamos las listas de alumnos resultado de sintprof y sintX

		try {
			Query1.comparaStudents(usuario, degree, subject, pStudents, xStudents, lang);
		}
		catch (ExcepcionChecker e) {
			cf = e.getCheckerFailure();
			cf.setUrl(call);
			cf.addMotivo(currentMethod+": ExcepcionChecker: "+MsgsEAMLChecker.getMsg("22",lang)+" students");
			throw new ExcepcionChecker(cf);
		}
	...
	public static void doGetC1CorrectAllForm (HttpServletRequest request, HttpServletResponse response, String lang) throws IOException
	{
		PrintWriter out = response.getWriter();
		int esProfesor = 1;
	...
	public static void doGetC1CorrectAllReport(HttpServletRequest request, HttpServletResponse response, String lang)
						throws IOException, ServletException
	{
		PrintWriter out = response.getWriter();
		int esProfesor = 1;
		...
		if (numCuentasP == null) {
			...
			CommonSINT.printEndPageChecker(out,  "13", esProfesor, MsgsEAMLChecker.CREATED, lang);
			return;
		}
		...
		for (int x=1; x <= numCuentas; x++) {
			...
			try {
				Query1.correctC1OneStudent(request, sintUser, Integer.toString(x), servicioAlu, passwdAlu, lang);
				bw.write("OK");
				bw.close();
				usersOK.add(x);
			}
	...
	public static void doGetC1CorrectAllForm2 (HttpServletRequest request, HttpServletResponse response, String lang) throws IOException
	{
		PrintWriter out = response.getWriter();
		int esProfesor = 1;
	...
	public static void doGetC1CorrectAllReport2(HttpServletRequest request, HttpServletResponse response, String lang)
						throws IOException, ServletException
	{
	    PrintWriter out;
		int esProfesor = 1;
		...
		if (numCuentasP == null) {
		   ...
		   out.println("<h4>Error: no se ha recibido el número de cuentas</h4>");
		   CommonSINT.printEndPageChecker(out,  "14", esProfesor, MsgsEAMLChecker.CREATED, lang);
		   return;
		}
		...
		try {
		    ...
		    RequestDispatcher dispatcher = CommonEAChecker.servletContextSintProf.getRequestDispatcher("/InformeResultadosCorreccionC1.jsp");
	...
	public static void doGetC1CorrectAllReport2Run(HttpServletRequest request, HttpServletResponse response, String lang)
						throws IOException, ServletException
	{
	...
	// para comparar el resultado de la F11: listas de degrees
			...
			throws ExcepcionChecker
	{
		...
		for (int x=0; x < pDegrees.size(); x++)
			if (!xDegrees.get(x).equals(pDegrees.get(x))) {
				cf = new CheckerFailure("", "20_DIFS", currentMethod+": "+String.format(MsgsEAMLChecker.getMsg("36",lang), pDegrees.get(x), x, xDegrees.get(x))); // diferencia degree: valor esperado, posicion, valor recibido
				throw new ExcepcionChecker(cf);
			}
	...
	// pide y devuelve la lista de subjects de un degree
	// levanta ExcepcionChecker si algo va mal

	private static ArrayList<Subject> requestSubjectsDegree (String call, String lang)
					throws ExcepcionChecker
	{
		String currentMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
		CheckerFailure cf;
		...
		if (CommonEAChecker.errorHandler.hasFatalerrors()) {
			...
			cf = new CheckerFailure(call, "", msg);
			cf.addMotivo(currentMethod+": "+String.format(MsgsEAMLChecker.getMsg("28",lang), "fatalerrors")+"subjects"); // resultado inválido, fatal errors en la lista de
			throw new ExcepcionChecker(cf);
		}
		...
		// procesamos todos los subjects

		for (int x=0; x < nlSubjects.getLength(); x++) {
			Element elemSubject = (Element)nlSubjects.item(x);
			String nombreSubject= elemSubject.getTextContent().trim();

			String curso = elemSubject.getAttribute("course");
			String tipo = elemSubject.getAttribute("type");

			listaSubjects.add(new Subject(nombreSubject, curso, tipo, "", "")); // idSub=grade="" ya que no nos hacen falta
		}
	...
	// para comparar el resultado de la F12: listas de subjects
	// no devuelve nada si son iguales
	// levanta ExcepcionChecker si hay diferencias

	private static void comparaSubjects (String usuario, String degreeActual, ArrayList<Subject> pSubjects, ArrayList<Subject> xSubjects, String lang)
			throws ExcepcionChecker
	{
		String currentMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
		CheckerFailure cf;
		String pNombre, xNombre, pCurso, xCurso, pTipo, xTipo;

		if (pSubjects.size() != xSubjects.size()) {
			cf = new CheckerFailure("", "20_DIFS", currentMethod+": SUBJECTS ("+usuario+"+"+degreeActual+"): "+Msgs.getMsg(Msgs.CC56,lang)+pSubjects.size()+" subjects, "+Msgs.getMsg(Msgs.CC57,lang)+xSubjects.size());  // CC56...CC57 = debería devolver...pero devuelve
			throw new ExcepcionChecker(cf);
		}
		...
		for (int y=0; y < pSubjects.size(); y++) {
			...
			if (!xNombre.equals(pNombre)) {
				cf = new CheckerFailure("", "20_DIFS", currentMethod+": SUBJECTS ('"+usuario+"', '" +degreeActual+"'): "+String.format(MsgsEAMLChecker.getMsg("37",lang), pNombre, y, xNombre)); // "Se esperaba el subject '%s' en la posición %d y se recibió '%s'"
				throw new ExcepcionChecker(cf);
			}

			pCurso = pSubjects.get(y).getCurso();
			xCurso = xSubjects.get(y).getCurso();

			if (!xCurso.equals(pCurso)) {
				cf = new CheckerFailure("", "20_DIFS", currentMethod+": SUBJECTS ('"+usuario+"', '"+degreeActual+"'): "+String.format(MsgsEAMLChecker.getMsg("38",lang), pCurso, y, xCurso)); // "Se esperaba el curso '%s' en la posición %d y se recibió '%s'"
				throw new ExcepcionChecker(cf);
			}

			pTipo = pSubjects.get(y).getTipo();
			xTipo = xSubjects.get(y).getTipo();

		  if (!xTipo.equals(pTipo)) {
				cf = new CheckerFailure("", "20_DIFS", currentMethod+": SUBJECTS ('"+usuario+"', '"+degreeActual+"'): "+String.format(MsgsEAMLChecker.getMsg("39",lang), pTipo, y, xTipo)); // "Se esperaba el tipo '%s' en la posición %d y se recibió '%s'"
				throw new ExcepcionChecker(cf);
		  }
		}
	...
	// pide y devuelve la lista de students de un subject de un degree
	// levanta ExcepcionChecker si algo va mal

	private static ArrayList<Student> requestStudentsSubjectDegree (String call, String lang)
									throws ExcepcionChecker
	{
		String currentMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
		CheckerFailure cf;
		...
		if (CommonEAChecker.errorHandler.hasFatalerrors()) {
			...
			cf = new CheckerFailure(call, "", msg);
			cf.addMotivo(currentMethod+": "+String.format(MsgsEAMLChecker.getMsg("28",lang), "fatalerrors")+"students"); // resultado inválido, fatalerrors en la lista de
			throw new ExcepcionChecker(cf);
		}
	...
	// para comparar el resultado de la F13: listas de students
	// no devuelve nada si son iguales
	// levanta ExcepcionChecker si hay diferencias

	private static void comparaStudents (String usuario, String degreeActual, String subjectActual, ArrayList<Student> pStudents, ArrayList<Student> xStudents, String lang)
				throws ExcepcionChecker
	{
		CheckerFailure cf;
		String currentMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
	    ...
	    if (pStudents.size() != xStudents.size()) {
			  cf = new CheckerFailure("", "20_DIFS", currentMethod+": STUDENTS ("+usuario+"+"+degreeActual+"+"+subjectActual+"): "+Msgs.getMsg(Msgs.CC56,lang)+pStudents.size()+" students, "+Msgs.getMsg(Msgs.CC57,lang)+xStudents.size()); // CC56...CC57 = debería devolver...pero devuelve
			  throw new ExcepcionChecker(cf);
	    }
	    ...
	    for (int z=0; z < pStudents.size(); z++) {
	    	    ...
	    	    if (!xNombre.equals(pNombre)) {
	    			    cf = new CheckerFailure("", "20_DIFS", currentMethod+": STUDENTS ('"+usuario+"', '"+degreeActual+"', '"+subjectActual+"'): "+String.format(MsgsEAMLChecker.getMsg("40",lang), pNombre, z, xNombre)); // "Se esperaba el student '%s' en la posición %d y se recibió '%s'"
					      throw new ExcepcionChecker(cf);
	    	    }
	    	    ...
	    	    if (!xId.equals(pId)) {
					      cf = new CheckerFailure("", "20_DIFS", currentMethod+": STUDENTS ('"+usuario+"', '"+degreeActual+"', '"+subjectActual+"'): "+String.format(MsgsEAMLChecker.getMsg("41",lang), pId, z, xId)); // "Se esperaba el ID '%s' en la posición %d y se recibió '%s'"
	    			    throw new ExcepcionChecker(cf);
	    	    }
	    	    ...
	    	    if (!xAddress.equals(pAddress)) {
	    			     cf = new CheckerFailure("", "20_DIFS", currentMethod+": STUDENTS ('"+usuario+"', '"+degreeActual+"', '"+subjectActual+"'): "+String.format(MsgsEAMLChecker.getMsg("42",lang), pAddress, z, xAddress)); // "Se esperaba la dirección '%s' en la posición %d y se recibió '%s'"
                 throw new ExcepcionChecker(cf);
						}
	    }
